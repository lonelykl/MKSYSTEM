...
$applicantEmail='';
$applicantContactNo='';
$stmt = $conn->prepare("select user_id,qualification_code,user_grade,qualification_type from lf_applicant_grade where comp_code = ? and user_id = ?");
...
while ( $stmt-> fetch() ) { 
$applicantID = $token2;
$applicantQualification = $token3;
$applicantGrade = $token4;
$qualificationGradingSystem = $token5;
}
$stmt->close();
...
function getGradeSubject() {
  ...
  xhttp.onreadystatechange = function() {
	...
	if(strColVal != document.getElementById("txtCurrentQualification").value){
...
function showhideDIVEdit(){
	...
	// this is to lock / unlock the field which disabbled
	document.getElementById("btnEdit").disabled = true;
	document.getElementById("drpQualification").disabled = false;
	document.getElementById("drpSubject").disabled = false;
	document.getElementById("txtScore").disabled = false;
	document.getElementById("btnAmend").disabled = false;
...
<td width="40%">
<input type="button" value="EDIT" name="btnEdit" id="btnEdit" onClick="showhideDIVEdit();amendGrade();">					
</td>
</tr>
</table>
</div>							
...
<td width="110%">
<select name="drpQualification" id="drpQualification" style="width: 100%;" onChange="getGradeSubject();" disabled>
...
while ( $stmt-> fetch() ) { 
?>
<option value="<?php echo $token2 ;?>" <?php if($applicantQualification == $token2){ ?> selected <?php } ?> ><?php echo $token3;?></option>
<?php
}
$stmt->close();
?>
</select>
<input type="hidden" name="txtCurrentQualification" id="txtCurrentQualification" class="form-control" placeholder="Enter Current Qualification" value="<?php echo $applicantQualification;?>" disabled>
<input type="hidden" name="txtCurrentGradingType" id="txtCurrentGradingType" class="form-control" placeholder="Enter Current Grading Type" value="<?php echo $qualificationGradingSystem;?>" disabled>
</td>
</tr>
</table>
</div>							
...
<td width="110%">
<input type="text" name="txtTextArea" id="txtTextArea" class="form-control" placeholder="Enter Applicant ID" value="<?php echo $applicantGrade;?>" disabled>
</td>
</tr>
</table>
</div>							
...
<input type="submit" name="btnSubmit" id="btnSubmit" value="Submit" class="btn btn-primary" onClick="unlockField();" style="display:none;"disabled>					
</div>	
</form>						
</div>		
</div>	
